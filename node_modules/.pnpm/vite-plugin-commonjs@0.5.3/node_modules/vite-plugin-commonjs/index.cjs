    ...
    if (topScopeNode) {
      switch (topScopeNode.type) {
        ...
        case TopScopeType.VariableDeclaration:
          ...
          const LV_str = (spe) => typeof LV === "object" ? LV.map((e) => e.key === e.value ? e.key : `${e.key} ${spe} ${e.value}`).join(", ") : "";
          if (init.type === "CallExpression") {
            ...
            const onlyOneMember = (_a = ancestors.find((an) => an.type === "MemberExpression")) == null ? void 0 : _a.property.name;
            const importDefault = onlyOneMember === "default";
            if (typeof LV === "string") {
              if (importDefault) {
                impt.importee = `import ${LV} from '${requireId}'`;
              } else {
                impt.importee = onlyOneMember === LV ? `import { ${LV} } from '${requireId}'` : `import { ${onlyOneMember} as ${LV} } from '${requireId}'`;
              }
            } else {
              if (importDefault) {
